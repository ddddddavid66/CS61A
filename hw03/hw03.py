...
def interleaved_sum(num, f_odd, f_even):
    ...
    def inner_even(i):
        if i == num:
            return f_even(i)
        return f_even(i) + inner_odd(i + 1)
    def inner_odd(i):
        if i == num:
            return f_odd(i)
        return f_odd(i) + inner_even(i + 1)
    if num < 1:
        return 0
    return inner_odd(1)
...
def count_dollars(sum_needed):
    ...
    def inner(sum,dollar):
        ...
        if dollar == None:
            return 0
        with_dollar = inner(sum - dollar,dollar)
        without_dollar = inner(sum,next_smaller_dollar(dollar))
        return with_dollar + without_dollar
    return inner(sum_needed,100)
...
def count_dollars_upward(sum_needed):
    ...
    def inner(sum,dollar):
        ...
        if dollar == None:
            return 0
        with_dollar = inner(sum - dollar,dollar)
        without_dollar = inner(sum,next_larger_dollar(dollar))
        return with_dollar + without_dollar
    return inner(sum_needed,1)
...
def knapsack(weights, values, c):
    ...
    if not weights or c == 0:
        return 0
    skip = knapsack(weights[1:], values[1:], c)
    if weights[0] <= c:
        take = values[0] + knapsack(weights[1:], values[1:], c - weights[0])
